        ...
        <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50">
                <tr>
                    ...
                    <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Salle de Classe</th>
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                ...
                <tr class="hover:bg-gray-40">
                    <td class="px-3 py-2 whitespace-nowrap">{{ $classe->name }}</td>
                    <td class="px-3 py-2 whitespace-nowrap">{{ $classe->description }}</td>
                    <td class="px-3 py-2 whitespace-nowrap">{{ $classe->classeroom->name ?? 'Non assigné' }}</td>
                    <td class="px-4 py-2">
                        <!-- Bouton Éditer -->
                        <button data-modal-target="edit-modal-{{ $classe->id }}" data-modal-toggle="edit-modal-{{ $classe->id }}"
                            class="text-blue-600 hover:text-blue-800 hover:underline">
                            ✏️
                        </button>

                        <!-- Modal de modification -->
                        <div id="edit-modal-{{ $classe->id}}" tabindex="-1" aria-hidden="true"
                                    ...
                                    <form method="POST" action="{{ route('classes.update', $classe->id) }}"
                                        ...
                                        <div>
                                            ...
                                            <textarea name="description" 
                                                class="w-full border border-gray-300 rounded-md shadow-sm p-2 focus:ring focus:ring-blue-300">{{ old('description', $classe->description) }}</textarea>
                                        </div>

                                        <div>
                                            <label class="block mb-1 font-semibold">Salle de Classe</label>
                                            <select name="classeroom_id"
                                                class="w-full border border-gray-300 rounded-md shadow-sm p-2 focus:ring focus:ring-blue-300">
                                                <option value="">Sélectionner une salle</option>
                                                @foreach($classerooms as $classeroom)
                                                    <option value="{{ $classeroom->id }}"
                                                        {{ old('classeroom_id', $classe->classeroom_id) == $classeroom->id ? 'selected' : '' }}>
                                                        {{ $classeroom->name }}
                                                    </option>
                                                @endforeach
                                            </select>
                                        ...
                                        <div class="flex justify-end space-x-4 pt-4 border-t border-gray-200 dark:border-gray-600">
                                            ...
                                            <button type="button" data-modal-hide="edit-modal-{{ $classe->id }}"
